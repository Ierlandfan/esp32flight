...
"const rc=L.circle([lat,lon],{radius:rkm*1000,weight:0,fill:false});"
"map.fitBounds(rc.getBounds(),{padding:[14,14]});"
...
"if(!layer)return;redraw=true;layer.clearLayers();"
...
"else m.on('click',clearRoute);"
"m.on('popupclose',()=>{if(!redraw&&selHex===f.hex)clearRoute();});"
